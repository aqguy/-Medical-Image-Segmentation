    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for client_idx in source_site_idx:
            ...
            for i_batch, sampled_batch in enumerate(dataloader_current):
                ...
                sampled_batch['label'], \
                    sampled_batch['disc_contour'], sampled_batch['disc_bg'], sampled_batch['cup_contour'], \
                ...
                fast_weights = OrderedDict(
                    (name, param - torch.mul(meta_step_size, torch.clamp(grad, 0 - clip_value, clip_value))) for
                    ((name, param), grad) in
