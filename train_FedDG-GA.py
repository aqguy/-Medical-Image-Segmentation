...
from dataset.worker_initializer import worker_init_fn
...
from utils.losses import refine_weight_dict_by_GA
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for client_idx in source_site_idx:
            ...
            for i_batch, sampled_batch in enumerate(dataloader_current):
                ...
                weight_dict = OrderedDict()  # Fill this with your initial weights
                site_before_results_dict = {}  # Fill with the results before adjustment
                site_after_results_dict = {}  # Fill with the results after adjustment
                ...
                fast_weights = OrderedDict(
                    (name, param - torch.mul(meta_step_size, torch.clamp(grad, 0 - clip_value, clip_value)))
                    for ((name, param), grad) in zip(net_current.named_parameters(), grads)
                )

                # Outer loop evaluation
                outputs_soft_outer_1, outputs_mask_outer_1, embedding_outer = net_current(volume_batch_trs_1,
                                                                                          fast_weights)  # alpha
                ...
                # Adjust weight_dict using GA-based refinement
                weight_dict = refine_weight_dict_by_GA(weight_dict, site_before_results_dict, site_after_results_dict,
                                                       step_size=0.1, fair_metric='loss')

                # Continue with the updated weight_dict
